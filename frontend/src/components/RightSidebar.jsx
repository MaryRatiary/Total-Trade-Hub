      ...
      {birthdays && birthdays.length > 0 && (
        <div className="sidebar-section">
          ...
          {birthdays.map((birthday, index) => (
            <div key={birthday.Id || birthday.id || `birthday-${index}`} className="birthday-item text-gray-700 mb-1">
      ...
      <div className="sidebar-section">
        ...
        <div className="contacts-list space-y-2">
          {suggestions?.map((contact, index) => (
            <div key={contact.Id || contact.id || `suggestion-${index}`} className="contact-item flex items-center gap-2 p-2 rounded hover:bg-gray-50">
              <img
              ...
              />
              <span className="flex-1 text-gray-800">{contact.FirstName || contact.firstName} {contact.LastName || contact.lastName}</span>
      ...
      <div className="sidebar-section">
        ...
        <div className="contacts-list space-y-2">
          {contacts?.map((contact, index) => (
            <div key={contact.Id || contact.id || `contact-${index}`} className="contact-item flex items-center gap-2 p-2 rounded hover:bg-gray-50">
              ...
              <span className="ml-2 text-gray-800">{contact.FirstName || contact.firstName} {contact.LastName || contact.lastName}</span>
            </div>
          ))}
        </div>
      </div>
