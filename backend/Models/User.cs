...
namespace TTH.Backend.Models // Ensure this namespace matches the one imported in Article.cs
{
    public class User
    {
        ...
        public string FullProfilePictureUrl => !string.IsNullOrEmpty(ProfilePicture) ? 
            (ProfilePicture.StartsWith("http") ? ProfilePicture : $"http://192.168.88.158:5131{ProfilePicture}") : 
            "http://192.168.88.158:5131/default-avatar.png";
        
        [BsonIgnore]
        public string FullCoverPictureUrl => !string.IsNullOrEmpty(CoverPicture) ? 
            (CoverPicture.StartsWith("http") ? CoverPicture : $"http://192.168.88.158:5131{CoverPicture}") : 
            "http://192.168.88.158:5131/defaults/default-cover.jpg";
